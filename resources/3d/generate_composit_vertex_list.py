def main (filename, ox=0, oy=0, oz=0, sx=1, sy=1, sz=1):
    ...
    name = '.'.join(filename.split('.')[0:-1]).upper()
    print "%s = pyglet.graphics.vertex_list(%d,('v3f/static', %s))" % (name, count, '('+(', '.join([str(n) for n in output_lists]))+')')
    objfile.close()
    
if __name__ == '__main__':
    main('Model_PayloadShield.obj')