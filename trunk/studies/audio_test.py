...
sound_pulse = bullet_sound = pyglet.media.StaticSource(pyglet.media.load('pulse.wav'))
...
class PlayerDroneModel (Model):
    ...
    def play_pulse (self):
        if self.sound_ref and self.sound_ref.time != 0:
            return
        self.sound_ref = sound_pulse.play()
        self.sound_ref.volume = .2
...
class Window (pyglet.window.Window):
    def __init__ (self, *args, **kwargs):
        ...
        self.models = []
        for n in xrange(14):
            self.models.append(DroneModel(n*-100, -100, -200 + n*-110))
        self.terrain = TerrainModel(0,self.height/-4)
        self.player = PlayerDroneModel(0, -100, 0)
